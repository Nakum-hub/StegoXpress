...
class VaultTab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # ── LEFT: Cover image ──
        left = W.card(self); left.grid(row=0, column=0, padx=(12, 6), pady=12, sticky="nsew")
        ...
        # ── MIDDLE: Payload ──
        mid = W.card(self); mid.grid(row=0, column=1, padx=6, pady=12, sticky="nsew")
        ...
        right = W.card(self); right.grid(row=0, column=2, padx=(6, 12), pady=12, sticky="nsew")
    ...
    def _start_encode(self):
        ...
        if decoy_pw != decoy_conf:
            self._set_status("Decoy passwords do not match.", COLORS["error"]); return
        if real_pw != real_conf:
            self._set_status("Real passwords do not match.", COLORS["error"]); return
        if decoy_pw == real_pw:
            self._set_status("Decoy and real passwords must differ.", COLORS["error"]); return
        if not decoy_pw or not real_pw:
            self._set_status("Both passwords required.", COLORS["error"]); return
        if not out_path:
            self._set_status("Set an output path.", COLORS["error"]); return
    ...
    def _start_decode(self):
        ...
        if not img_path or not os.path.exists(img_path):
            self._set_decode_result("Image not found.", COLORS["error"]); return
        if not password:
            self._set_decode_result("Enter a password.", COLORS["error"]); return
        threading.Thread(
        ...
        try:
            while True:
                ...
                if msg[0] == "encode_ok":
                    ...
                    if zone == "inner":
                        lbl, col = "🔐  REAL MESSAGE — inner vault", COLORS["accent"]
                    else:
                        lbl, col = "🎭  DECOY MESSAGE — outer vault", COLORS["text_muted"]
